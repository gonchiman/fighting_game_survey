...
class GameTitle(Enum):
    ...
    TEKKEN_7 = GameData(
        "TEKKEN 7",
        389730,
        DATA_DIR / "tekken_7.csv"
    )

    DRAGON_BALL_FIGHTERZ = GameData(
        "DRAGON BALL FighterZ",
        678950,
        DATA_DIR / "dragon_ball_fighterz.csv"
    )

    SOULCALIBUR_VI = GameData(
        "SOULCALIBUR VI",
        544750,
        DATA_DIR / "soulcalibur_vi.csv"
    )

    MORTAL_KOMBAT_11 = GameData(
        "Mortal Kombat 11",
        976310,
        DATA_DIR / "mortal_kombat_11.csv"
    )

    GUILTY_GEAR_STRIVE = GameData(
        "GUILTY GEAR -STRIVE-",
        1384160,
        DATA_DIR / "guilty_gear_strive.csv"
    )

    THE_KING_OF_FIGHTERS_XV = GameData(
        "THE KING OF FIGHTERS XV",
        1498570,
        DATA_DIR / "the_king_of_fighters_xv.csv"
    )
    ...
    MORTAL_KOMBAT_1 = GameData(
        "Mortal Kombat 1",
        1971870,
        DATA_DIR / "mortal_kombat_1.csv"
    )

    GRANBLUE_FANTASY_VERSUS_RISING = GameData(
        "Granblue Fantasy Versus: Rising",
        2157560,
        DATA_DIR / "granblue_fantasy_versus_rising.csv"
    )
